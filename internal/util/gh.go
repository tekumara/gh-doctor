...
func ExecGhInteractive(args ...string) error {
	...
	cmd.Env = append(ghEnv, os.Environ()...)
...
// ExecGh invokes a gh command in a subprocess and captures stdout and stderr
func ExecGh(args ...string) (stdout []byte, err error) {
	ghExe, err := gh.Path()
	if err != nil {
		return
	}
	...
	if ee, ok := err.(*exec.ExitError); ok {
		return stdout, fmt.Errorf("%s", ee.Stderr)
	}
	return
}
